...
function get_post_ancestors($post)
{
	...
	while ($ancestor = get_post($id)) {
		...
		if (
			empty($ancestor->post_parent) || $ancestor->post_parent === $post->ID
...
function wp_check_for_changed_slugs($post_id, $post, $post_before)
{
	...
	if (
		! ('publish' === $post->post_status || ('attachment' === $post->post_type && 'inherit' === $post->post_status))
...
function _transition_post_status($new_status, $old_status, $post)
{
	...
	if ($new_status !== $old_status && isset($post) && isset($post->post_type)) {
